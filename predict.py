...
def get_col1(fn):
    finame = r'E:/Omics/Kla/transfer/10features_for_experiment/' + str.upper(fn) + r'/A0JPQ4_130.' + str.lower(fn)
...
def get_name_and_label():
    df = pd.read_csv('E:/Omics/Kla/transfer/DNN_result_for_experiment/ASA_scores.csv')
    name = df['pepname']
    name = name.values
    return name
...
for feature in feature_list:
    model = load_model(f'E:/Omics/Kla/transfer/models/DNN/%s/%s_DNN_%d.h5' % (feature, feature, random.randint(1, 3)))
    dataset = pd.read_csv(f'E:/Omics/Kla/transfer/1D_dataset_for_experiment/%s_dataset.csv' % feature)
    ...
    path = f'E:/Omics/Kla/transfer/DNN_result_for_experiment/'
    if not os.path.exists(path):
        os.makedirs(path)
    df_y.to_csv(path + f'%s_scores.csv' % feature, index=False)

for count in range(1, 4):
    data = auc_11('E:/Omics/Kla/transfer/DNN_result_for_experiment')
    iDNN = load_model(f'E:/Omics/Kla/models/integrated_DNN/iDNN_%d.h5' % count)
    score = iDNN.predict(data)
    peplist = get_name_and_label()
    df_y = pd.concat([pd.DataFrame(list(peplist), columns=['pepname']),
                  pd.DataFrame(list(score), columns=['score'])], axis=1)
    df_y.to_csv(f'E:/Omics/Kla/transfer/iDNN_%d_result_for_experiment.csv' % count, index=False)
