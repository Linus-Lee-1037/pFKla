...
for model_type in model_list:
    for feature in feature_list:
        ...
        model1 = get_model('./models/', feature, model_type)
        ...
        train_time = 5
        split = 3
        ROC_threshold = 0.7  # 0.7 for transformer's DNN ;  0.55 for the rest
        ...
        # while train_time < 100:
            ...
            if train_time > 100:  # or ROCAUC_score < 0.45:
                train_time = 5
                model1 = get_model('E:/Omics/Kla/models', feature, model_type)
                for layer in model1.layers[:-3]:
                    layer.trainable = False
            # '''
            # train_time += random.randint(1, 10)
...
with open(r'./transfer/models/DNN/reverse_list.txt', mode='w') as textfile:
    for reverse_model in reverse_models:
        textfile.write(reverse_model + '\n')
for le in less_than_50:
    print(le)
if os.path.exists('./transfer/DNN_train_time.csv'):
    original_training_time_record = pd.read_csv('./transfer/DNN_train_time.csv')
    for record_name in training_time_record.keys():
        original_training_time_record[record_name] = training_time_record[record_name]
    original_training_time_record.to_csv('./transfer/DNN_train_time.csv', index=False)
else:
    pd.DataFrame(training_time_record, index=[0]).to_csv('./transfer/DNN_train_time.csv', index=False)
