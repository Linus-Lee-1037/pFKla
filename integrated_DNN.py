...
def training_DNN(pepID, x, y):
    ...
    for train_index, test_index in skf.split(x, y):
        ...
        dnn.fit(x_train, y_train, epochs=40, batch_size=2048)
...
while count < 3:
    ...
    if not os.path.exists(r'./models/integrated_DNN'):
        os.makedirs(r'./models/integrated_DNN')
    #if auc_score >= max(AUCs):
    df_y.to_csv(f'./models/integrated_DNN/iDNN_%d_y_label&score.csv' % count, index=False)

print(f'%s模型的auc分数为：' % 'iDNN', max(AUCs))
highest_AUC_scores['function_Kla'] = max(AUCs)
model_s[AUCs.index(max(AUCs))].save(f'./models/integrated_DNN/iDNN_%d.h5' % AUCs.index(max(AUCs)))
# '''
'''
...
pd.DataFrame(df_scores).to_csv('./transfer/scores.csv', index=False)
# '''
