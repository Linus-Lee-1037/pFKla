...
#draw_ROC
df_iDNN = pd.read_csv('E:/Omics/Kla/transfer/models/integrated_DNN/iDNN_y_label&score.csv')
label = df_iDNN['label']
score = df_iDNN['score']
...
plt.plot(iDNN_fpr, iDNN_tpr, linewidth='3', label='iFKla (AUC = %.3f)' % iDNN_auc, color='red')
...
for i, featurename in enumerate(feature_list):
    if featurename != 'Transformer':
        df = pd.read_csv(f'E:/Omics/Kla/models/%s/%s/%s_y_label&score.csv' % (modelname, str.upper(featurename), str.upper(featurename)))
    else:
        df = pd.read_csv(f'E:/Omics/Kla/models/%s/%s/%s_y_label&score.csv' % (modelname, str.lower(featurename), str.lower(featurename)))
...
plt.savefig('E:/Omics/figure/pre-training_AUC.png', dpi=300)
#plt.show()