...
def get_col(fn):
    finame = r'./10features/' + str.upper(fn) + r'/Q99961_76.' + str.lower(fn)
...
def training_DNN(feature_name):
    ...
    for train_index, test_index in skf.split(x, y):
        ...
        dnn.fit(x_train, y_train, epochs=50, batch_size=2048)

        y_label.append(list(y_test))
        y_test_score = dnn.predict(x_test)
        y_score.append(list(y_test_score))
        # models.append(dnn)
        dnn.save(r'./models/DNN/' + str.upper(feature_name) + r'/' + str.upper(feature_name) + r'_DNN_' + str(count) +
                 r'.h5')
...
for feature in features:
    dataset = pd.read_csv(r'./1D_dataset/' + feature + r'_dataset.csv')
